...
//  ДЗ6.2.1
        int[] dailyExpenses;
        System.out.println();
        int dayInMonth = 30, minExpenses = 100_000, maxExpenses = 200_000, rangeExpenses = maxExpenses - minExpenses + 1;
...
/*    static void showArray(int[] someArray) {
        for (int i = 0; i < someArray.length - 1; i++) {
            System.out.printf("%s, ", someArray[i]);
        }
    ...
    static int calculateSumm(int[] expensesArray) {
        int summ = 0;
        for (int currentElement : expensesArray) {
            summ += currentElement;
        }
        return summ;
    }
    ...
    static int minDaylyExpenses(int[] dailyExpenses) {
        ...
        for (int curentElement : dailyExpenses) {
            if (curentElement < min) {
                min = curentElement;
            }
        }
        return min;
